...
int generateBOWImageDescsTrain(std::string dataPath,
...
int generateBOWImageDescsTest(std::string dataPath,
							std::string bowImageDescPath,
							std::string vocabPath,
							cv::Ptr<cv::FeatureDetector> &detector,
							cv::Ptr<cv::DescriptorExtractor> &extractor,
...
int main(int argc, char * argv[])
{
	...
	if (argc == 1) {
        ...
        settfilename = "../../test/settings.yml";
	} else if (argc == 3) {
	...
	if (function == "ShowFeatures") {
		...
		result = generateBOWImageDescsTrain(fs["FilePaths"]["TrainPath"],
		...
		result = generateBOWImageDescsTest(fs["FilePaths"]["TestPath"],
		...
		if(fabmap) {
			result = openFABMAP(fs["FilePaths"]["TestImageDesc"], 
				fs["FilePaths"]["TrainImagDesc"], fabmap,
...
int showFeatures(std::string trainPath, cv::Ptr<cv::FeatureDetector> &detector)
{
	...
	//open the frames
	char * TRAIN_PATH = new char[100];
	for (int i = 1; i <=29; ++i)
		for (int j = 0; j < 8; ++j)
		{
			sprintf(TRAIN_PATH,"%s%d%s%d%s","pano_" , i , "_rect_", j, ".png");
	
			cv::Mat frame = cv::imread(trainPath+TRAIN_PATH,CV_LOAD_IMAGE_GRAYSCALE);
						   ...
						   cv::Ptr<cv::DescriptorExtractor> &extractor)
{
	...
	for (int i = 1; i <=29; ++i)
		...
		std::cout << "We obtain " << vocabTrainData.rows << " descriptors         \r";
		fflush(stdout); 
							...
							int minWords)
{
	...
	for (int i = 1; i <=29; ++i)
		for (int j = 0; j < 8; ++j)
		{
			...
			cv::Mat frame = cv::imread(dataPath+TRAIN_PATH,CV_LOAD_IMAGE_GRAYSCALE);
							...
							int minWords)
{
	...
	char * TEST_PATH = new char[100];
	for (int i = 0; i <=891; ++i)
	{
		sprintf(TEST_PATH,"%s%d%s","vedecom_",i,".jpg");
	
		cv::Mat frame = cv::imread(dataPath+TEST_PATH,CV_LOAD_IMAGE_GRAYSCALE);
			   ...
			   bool addNewOnly)
{
	...
	//load the train data
	fs.open(trainPath, cv::FileStorage::READ);
	cv::Mat trainImageDescs;
	fs["BOWImageDescs"] >> trainImageDescs;
	if(trainImageDescs.empty()) {
		std::cerr << trainPath << ": Test data not found" << std::endl;
		return -1;
	}
	fs.release();
	...
	std::cout << trainImageDescs.rows << std::endl;

	cv::Mat confusion_mat(testImageDescs.rows, trainImageDescs.rows, CV_64FC1);
    confusion_mat.setTo(0); // init to 0's
   
    fabmap->compare(testImageDescs, trainImageDescs, matches);
